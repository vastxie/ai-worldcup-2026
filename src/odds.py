...
H2H_MAX_AGE = 90 * 60          # 逐场市场参考缓存 90 分钟
WINNER_MAX_AGE = 20 * 3600     # 夺冠市场参考缓存 20 小时
...
def _total_from_market(outcomes: list[dict]) -> float | None:
    ...
    足球 totals 常以 2.5/3.0 等市场参考给出。这里保守地用市场参考点位作为中心，
...
def _fetch_winner(key: str) -> dict:
    """夺冠市场概率：{code: p}（去水后按数据源取中位数，整体归一）。"""
...
def sync(quiet: bool = False, dry_run: bool = False) -> dict | None:
    ...
    try:
        ...
        if now - cache.get("h2h_ts", 0) > H2H_MAX_AGE:
            ...
            if not quiet:
                print(f"  [odds] 已更新 {len(cache['h2h'])} 场市场参考")
        if now - cache.get("winner_ts", 0) > WINNER_MAX_AGE:
            ...
            if not quiet:
                print(f"  [odds] 已更新夺冠回报系数（{len(cache['winner'])} 队）")
        if not dry_run:
